...
\begin{abstract}
Modern scientific simulators are configured through executable interfaces --- XML decks, input scripts, namelists, configuration dictionaries --- that function as domain-specific languages: structured by elements, attributes, and cross-section constraints and authored against extensive, fragmented documentation. Translating a researcher's natural-language intent into a runnable configuration is a recurring expert bottleneck, and realistic subsurface studies routinely require many simulations across geological realizations, parameter sweeps, and engineering objectives. We treat scientific-simulation setup as a \emph{translate-the-manual-into-the-DSL} problem and study, empirically, how much of the bottleneck a general-purpose LLM coding harness (Claude Code) can absorb when wrapped with a \textbf{Simulator Interface Adapter (SIA)}: four simulator-agnostic components --- interface retrieval, an always-on interface primer, an output verifier, and bounded repair hints --- instantiated for GEOS, an open-source multiphysics simulator used in CO$_2$ storage, geothermal, and induced-seismicity research. We measure each component in isolation and in combination via a Resolution-IV factorial, then probe two harder regimes: \textbf{(i) what happens when the user specifies less} and is offered a simulated-human consultation channel, and \textbf{(ii) how the harness compares to two PhD-level geoscience volunteers} attempting a representative benchmark task under a one-hour budget.

Three empirical findings. \textbf{(1)} Across the design-space cells, performance clusters within seed noise on the in-distribution test bench; on a stricter held-out evaluation set the cells spread, but the spread is a hard-tail effect concentrated in a small number of compound multi-physics tasks where adapters prevent catastrophic failure (${\sim}40{\times}$ seed-variance reduction) rather than raising means. The only main effect that clears noise on the in-distribution set is generic retrieval, and it is \emph{negative}. \textbf{(2)} The volunteers complete only the first of two required files in ${\sim}50{\times}$ the agent's wall-clock and score below the agent's structural-similarity mean on that single file, while their browser histories show dozens of GEOS-documentation visits each; the agent, with no internet, instead navigates the source-tree example library (vanilla harness) or runs structured retrieval (SIA). \textbf{(3)} When the user specifies less and is given an explicit human-consultation channel, the agent consults the simulated supervisor in only ${\sim}3\%$ of trials, and prompt framing does not move the rate --- tracing shows the on-disk example library functions as a cheaper retrieval substitute for the human channel. We close with adapter-design recommendations grounded in the failure categories that survive the vanilla$\to$best transition.
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Cell} & \textbf{test-17 TreeSim} & \textbf{$\sigma$} & \textbf{Held-out-10 TreeSim} & \textbf{$\sigma$} & \textbf{$\Delta$ vs Vanilla (test-17)} & \textbf{$\Delta$ vs Vanilla (Held-out-10)} \\
    \midrule
...
The eight factorial cells on test-17 yield the per-factor main effects in Table~\ref{tab:main-effects}, estimated as the mean TreeSim difference between the four cells with the factor on and the four with it off. R is the only factor whose magnitude exceeds the seed-noise floor; X and M are positive but small; S is essentially zero. On test-17 alone the design implication is ``don't add RAG; the rest doesn't matter.'' The Held-out-10 results above already show this conclusion is benchmark-specific.

\begin{table}[h]
  \caption{Resolution-IV main effects on test-17 TreeSim (\texttt{deepseek-v4-flash}, $n=3$ per cell).}
...
\paragraph{Outcome and time.} Both participants ran out of time. P1 stopped at $48$ minutes; P2 stopped at $47$ minutes. Both had completed only the first of the two required files (\texttt{buckleyLeverett\_base.xml}) and reported that they had spent the bulk of the second half of the hour trying to set up the \texttt{Outputs} and \texttt{Events} blocks against partially understood documentation. Neither attempted the second file (\texttt{buckleyLeverett\_benchmark.xml}). Scoring the partial decks gives Table~\ref{tab:human-baseline}: \emph{file-level} TreeSim against the GT base file is $0.812$ (P1) and $0.781$ (P2); \emph{deck-level} TreeSim against the full GT directory is $0.540$ and $0.527$ respectively, because the second file is missing. The agent's vanilla Claude Code on \texttt{minimax-m2.7} reaches TreeSim ${\approx}\,0.87$ on the same task in ${\approx}\,5$ minutes wall, and the SIA cell at the test-17 best corner (X+M, Vanilla CC + xmllint MCP + memory cheatsheet) reaches a higher number still under the same wall-clock budget. The wall-clock ratio is therefore approximately $10\times$ on the file-level comparison and $50\times$+ on the full-deck comparison, and the agent's quality on the file P1/P2 actually completed is already above either's.
