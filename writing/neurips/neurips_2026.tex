...
A second motivation runs alongside the adapter-design question. The benchmark in its main form hands the agent a fully detailed brief, which in practice strips the autonomy out of the task: the agent is asked to translate, not to decide. Yet a useful scientific-software assistant has to make decisions in the under-specified regime --- choosing solver settings, filling in standard material properties, and deciding when to ask the human researcher rather than guess. We use the same harness, on the same tasks, with deliberately relaxed specifications and an explicit human-consultation channel, to measure the ``decide when to consult'' capability that an autonomous-discovery setting would require (\S\ref{subsec:autonomy}). The result is mostly a measurement of how rarely the harness initiates consultation when the affordance exists, and why.

\paragraph{Contributions.}
A 17-task held-out test set (\emph{test-17}) and a 10-task held-out evaluation set (\emph{Held-out-10}) for GEOS deck authoring, scored by a tree-aware structural similarity metric (TreeSim) under failures-as-zero aggregation (fa0); a Resolution-IV $2^{4-1}$ factorial over the four adaptation factors plus the SE monolith, evaluated at $n=3$ seeds on \texttt{deepseek-v4-flash}; a per-task bottleneck-analysis pipeline (DSv4-flash classifier $+$ DSv4-pro synthesis) that decomposes adapter wins into surviving failure categories; four concrete adapter-design recommendations grounded in the categories that survive the Vanilla$\to$best transition; and an autonomy companion study (\S\ref{subsec:autonomy}) measuring how often the harness elects to consult a simulated human researcher when the specification is deliberately under-specified --- we find a $3\%$ consultation rate that is invariant to prompt framing, and trace it to the on-disk example library acting as a retrieval substitute. Negative results we judge worth reporting: retrieval-based memory exposed via MCP is never invoked, RAG is the only main effect that clears noise on test-17 and is \emph{negative}, the number of strictly-perfect decks does not increase under any adapter we tested, and a working human-consultation channel is largely unused.
...
A natural follow-up to the main study is to ask how the harness behaves when the user specifies \emph{less}. Our headline benchmark hands the agent a fully detailed natural-language brief --- domain, mesh, every material constant, every solver tolerance, every output schedule --- which collapses the authoring task to translation. A more ecologically realistic setting is one in which the user provides only the problem-defining details and expects the agent to infer or to ask. We probe this regime with a small, single-seed companion study (8 tasks drawn from test-17, two configurations, two relaxation levels, two interaction modes; 64 runs total in $\sim$2h wall and ${<}\$5$ DSv4 spend). Full protocol in App.~\ref{app:autonomy}.

\paragraph{Setup.} For each task we use \texttt{deepseek-v4-pro} to produce two reduced briefs from the original \texttt{instructions.txt}: \textbf{Medium} omits T1 (software defaults: output format, restart, log levels) and T2 (standard numerics: solver tolerances, time-step limits, discretisation choices); \textbf{Hard} additionally omits T3 (domain-inferable physical values: densities, viscosities, porosities, permeabilities, standard boundary conditions). The rewrite is fluent, leakage-checked numerically, and verified to preserve every T4 problem-defining value verbatim. Across the 8 tasks, Medium drops 89 parameter-level values ($-21\%$ chars on average); Hard drops 184 (including 105 T3 values; $-50\%$ chars). We then evaluate two configurations --- Vanilla (the F0 baseline) and X+M (the test-17 best cell) --- under two interaction modes: \textbf{non-interactive}, identical to the main pipeline; and \textbf{interactive}, in which the agent is given a \texttt{consult\_supervisor(question)} MCP tool whose handler invokes a separate \texttt{deepseek-v4-flash} instance prompted with the \emph{full} original brief and instructed to answer concisely without volunteering detail.
