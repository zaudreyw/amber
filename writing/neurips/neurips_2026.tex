...
\begin{abstract}
Modern scientific simulators are configured through executable interfaces (XML decks, input scripts, namelists) that function as domain-specific languages whose vocabulary is the simulator's internal API and whose grammar is documented across extensive, fragmented references. Translating a researcher's natural-language intent into a runnable configuration is a recurring expert bottleneck, repeated across the many simulations required for a realistic study. We empirically study how much of that bottleneck a general-purpose LLM coding harness (Claude Code) can absorb when wrapped with a \textbf{Simulator-Interface Grounding Adapter (SIGA)}: a bespoke package of skills, tools, and workflow-altering control flow that grounds the agent's outputs in the simulator's documentation, schema, and example library. As a case study, we instantiate SIGA for GEOS, an open-source multiphysics simulator used in CO$_2$-storage and induced-seismicity research. A Resolution-IV factorial over four SIGA components and a self-evolved monolithic variant surfaces three axes of benefit. \textbf{Reliability:} SIGA reduces across-seed variability by ${\sim}40{\times}$ relative to the vanilla harness by preventing the agent from producing unparseable or empty decks on a hard tail of compound multi-physics tasks. \textbf{Quality:} on the same hard tail, the best SIGA configuration lifts mean structural similarity by $7$ percentage points over the vanilla harness. \textbf{Efficiency:} a self-evolved monolithic variant --- adapter contents iteratively rewritten by an offline pipeline rather than designed by hand --- matches our best manually-designed configuration on quality at ${\sim}16\%$ fewer tool calls per task than vanilla Claude Code, a preliminary signal that automatic SIGA discovery is a tractable follow-up to the manual design space we explore here. We additionally contribute a behavioural finding about how a coding-agent harness handles deliberately under-specified briefs: with a \texttt{consult\_supervisor} tool exposed --- backed by a separate LLM instance with access to the original full brief, simulating a domain expert --- the agent invokes the channel in only ${\sim}3\%$ of trials regardless of prompt framing, instead using the on-disk example library as a cheaper retrieval substitute. A preliminary human baseline finds PhD-level geoscience volunteers take more than $8\times$ as long as the SIGA-equipped agent to complete a representative task and produce lower-quality decks. We close with SIGA-design recommendations grounded in the failure modes that remain unresolved even under our best configuration.
\end{abstract}
...
\paragraph{Outcome and time.} Both participants ran out of time. P1 stopped at $48$ minutes; P2 stopped at $47$ minutes. Both had completed only the first of the two required files (\texttt{buckleyLeverett\_base.xml}) and reported that they had spent the bulk of the second half of the hour trying to set up the \texttt{Outputs} and \texttt{Events} blocks against partially understood documentation. Neither attempted the second file (\texttt{buckleyLeverett\_benchmark.xml}). Scoring the partial decks gives Table~\ref{tab:human-baseline}: \emph{file-level} TreeSim against the GT base file is $0.812$ (P1) and $0.781$ (P2); \emph{deck-level} TreeSim against the full GT directory is $0.540$ and $0.527$ respectively, because the second file is missing. The agent's vanilla Claude Code on \texttt{minimax-m2.7} reaches TreeSim ${\approx}\,0.87$ on the same task in ${\approx}\,5$ minutes wall, and the SIGA cell at the val best corner (X+M, Vanilla CC + xmllint MCP + memory cheatsheet) reaches a higher number still under the same wall-clock budget. The wall-clock ratio is therefore approximately $10\times$ on the file-level comparison and $50\times$+ on the full-deck comparison, and the agent's quality on the file P1/P2 actually completed is already above either's.
