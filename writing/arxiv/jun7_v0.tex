...
Advanced scientific simulators expose specialized input languages that turn simulation goals into executable configurations, but learning and using these interfaces can cost domain scientists hours to days. We study simulator setup as a problem of agent-tool interface grounding: what minimal simulator-specific adaptations are needed for an off-the-shelf coding agent to operate real scientific software? Our intuition is that coding agents already know how to navigate files, edit code, run commands, and repair outputs, but they lack the simulator's executable contract: its vocabulary, structural constraints, validation rules, and termination conditions. We introduce SIGA, a Simulator-Interface Grounding Adapter that supplies this contract through retrieval, procedural memory, in-trajectory validation, and validation-enforced termination. We primarily study GEOS, an open-source multiphysics simulator used in subsurface science. In a human baseline case study, SIGA reaches quality parity with an human expert's 3 hour effort in 5 minutes, representing $\sim 36\times$ speedup. On a harder held-out set, grounding raises the mean quality score by roughly $10\%$ relative to the bare agent and cuts score variance by about $16\times$. 
% We further show that an open-ended self-evolution pipeline, in which the agent edits its own tools, memories, and prompts rather than drawing from a fixed component set, automatically enhances an adapter to exceed the best hand-designed configuration on this held-out set. 
We further show that a self-evolution mechanism (in which the agent edits its own harness) can enhance an agent to exceed hand-designed configurations on this held-out set. 
Finally, we additionally explore generalization on two more simulators, OpenFOAM and LAMMPS, where SIGA again improves performance and where different modules prove most effective according to each simulator's dominant challenge. Together these results suggest that a lightweight, self-improvable grounding layer is enough to make a general coding agent a reliable operator of scientific software, and offers a practical recipe for adapting it to new simulators.
